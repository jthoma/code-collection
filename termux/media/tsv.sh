...
fc=1; for i in f/*.png ; do magick $i -gravity northeast -background none -pointsize 50 -annotate +8+8 "$dtns:$sec" tsm.png; mv -vf tsm.png $i ; fc=$(echo " $fc + 1 " | bc); echo "$fc $sec $i " ; if [ "$fc" -eq 31 ]; then sec=$( echo " $sec + 1 " | bc); fc=1 ;fi ; done
